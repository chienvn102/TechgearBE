...
class CloudinaryService {
  ...
  // ✅ UPLOAD BANNER IMAGE: Specialized banner image upload với 1920x1080 format
  async uploadBannerImage(file, bannerId) {
    try {
      const {
        folder = CLOUDINARY_FOLDERS.BANNERS,
        public_id_prefix = `banner_${bannerId}`,
        resource_type = 'image'
      } = {};

      // Upload với banner-specific transformations
      const uploadOptions = {
        folder: folder,
        public_id: `${public_id_prefix}_${Date.now()}`,
        resource_type: resource_type,
        ...
        quality: 'auto',
        eager: [
          TRANSFORMATIONS.BANNER_THUMBNAIL,
          TRANSFORMATIONS.BANNER_MEDIUM,
          TRANSFORMATIONS.BANNER_LARGE
        ],
        eager_async: true,
        use_filename: false,
        unique_filename: true
  ...
  // ✅ GET BANNER URLS: Lấy URLs cho banner với format 1920x1080
  getBannerUrls(public_id, options = {}) {
    try {
      const urls = {
        original: cloudinary.url(public_id, { 
          ...TRANSFORMATIONS.ORIGINAL,
        ...
        display: cloudinary.url(public_id, { 
          ...TRANSFORMATIONS.BANNER_DISPLAY,
          ...options 
        })
      };
      ...
      console.error('❌ Error generating banner URLs:', error);
      throw new Error(`Failed to generate banner URLs: ${error.message}`);
    }
  }
